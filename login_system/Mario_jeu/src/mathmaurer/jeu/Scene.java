...
public class Scene extends JPanel {
	...
	public Scene(){		
		...
		this.xFond1 = -50; // L'image de fond depasse des 2 cetes de la fenetre.
		this.xFond2 = 750;
		this.ok = true;
		...
		// Mettre a la fin du constructeur
		Thread chronoEcran = new Thread(new Chrono()); // Ajout d'un chronometre a PanJeu
		chronoEcran.start(); // Demarrage du chronometre (appel de la methode run() de la classe Chrono)
	}
	...
	//**** METHODES ****//
	public void deplacementFond() { // Deplacement du fond lorsque mario se deplace
		if (this.xPos >= 0 && this.xPos <= 4430) {
			this.xPos = this.xPos + this.dx;
			this.xFond1 = this.xFond1 - this.dx;
			this.xFond2 = this.xFond2 - this.dx;			
		// Remise a zero des abscisses pour rotation des images de fond
	...
	public void paintComponent(Graphics g) {
    	...
    	// Detection des contacts de mario avec des pieces
 		...
 		// Deplacement de tous les objets "fixes" du jeu et des personnages (hors mario)
	    ...
	    g2.drawString(this.score.getNbrePieces() + " piece(s) founded " + this.score.getNBRE_TOTAL_PIECES(), 460, 25);
	    
	    // Fin de partie
	    if(this.finDePartie() == true){
	    	Font policeFin = new Font("Arial", Font.BOLD, 50);
            g2.setFont(policeFin);
	        if(this.partieGagnee() == true){g2.drawString("you have won!!", 120, 180);}
	        else{g2.drawString("  you have lost...", 120, 180);}
 	    }
 	}}